  ...
  try {
    ...
    for (const [categoryId, categoryData] of Object.entries(categories)) {
      ...
      for (const service of data.services) {
        ...
        try {
          const docRef = await addDoc(collection(firestore, 'services'), validService);
          console.log(`Successfully migrated service: ${validService.name} with ID: ${docRef.id}`);
        } catch (e) {
          console.error('Error adding document:', e);
          console.error('Failed service data:', JSON.stringify(validService, null, 2));
        }
      }
    }
